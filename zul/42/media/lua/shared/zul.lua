...
function ZUL.loadSandboxOptions(force)
    ...
    if ZUL.sandboxOptions.loaded and not force then
        return
    end
...
local function _log(level, modName, message, context, details, usePrint)
    ...
    elseif context then
        if type(context) == "table" then
            fullMessage =
                string.format("%s [%s] %s - %s", side, levelName, message, ZUL.serialize(context))
        else
            fullMessage =
                string.format("%s [%s] %s %s", side, levelName, message, tostring(context))
        end
...
function ZUL.new(modName)
    ...
    function child:trace(msg, ctx, det)
        ZUL.trace(modName, msg, ctx, det)
    end

    function child:debug(msg, ctx, det)
        ZUL.debug(modName, msg, ctx, det)
    end

    function child:info(msg, ctx, det)
        ZUL.info(modName, msg, ctx, det)
    end

    function child:warn(msg, ctx, det)
        ZUL.warn(modName, msg, ctx, det)
    end

    function child:error(msg, ctx, det)
        ZUL.error(modName, msg, ctx, det)
    end

    function child:fatal(msg, ctx, det)
        ZUL.fatal(modName, msg, ctx, det)
    end

    function child:log(msg, ctx, det)
        ZUL.log(modName, msg, ctx, det)
    end

    function child:setLevel(level)
        ZUL.setLevel(modName, level)
    end

    function child:getEffectiveLevel()
        return ZUL.getEffectiveLevel(modName)
    end

    function child:isLoggingEnabled(level)
        return ZUL.isLoggingEnabled(modName, level)
    end
