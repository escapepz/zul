...
The SharedLogger has been successfully refactored into **ZUL (Zomboid Unified Logging)** framework with full sandbox options support and robust multiplayer synchronization.
...
### 1. Robust Multiplayer Synchronization ✅

- **Staged Initialization**: Hooks into 5 key PZ events (`OnGameBoot`, `OnInitGlobalModData`, `OnInitWorld`, `OnGameStart`, `OnServerStarted`).
- **Forced Refreshes**: Later events trigger a refresh of sandbox settings to catch delayed server synchronization.
- **State-Aware Logging**: Differentiates between initial setup and server-synced refreshes in logs.
